...
const FONTS = [
  ...
  { id: "pen",      label: "손글씨체",   css: "'Nanum Pen Script', cursive" },
  { id: "brush",    label: "붓글씨체",   css: "'Nanum Brush Script', cursive" },
  { id: "square",   label: "나눔스퀘어", css: "'Nanum Square', sans-serif" },
  { id: "gmarket",  label: "지마켓Sans", css: "'GmarketSans', sans-serif" },
  { id: "black",    label: "검은고딕",   css: "'Black Han Sans', sans-serif" },
...
const EMOJI_SETS = [
  ...
  { id: "summer", label: "여름", icon: "☀️", emojis: ["☀️","🍉","🏖️","🌊","🐠","🌻","🦕","🍦"] },
  { id: "autumn", label: "가을", icon: "🍁", emojis: ["🍁","🍂","🌰","🎃","🍄","🦔","🍎","🐿️"] },
  { id: "winter", label: "겨울", icon: "❄️", emojis: ["❄️","⛄","🧣","🎄","🐧","🌨️","🧤","🦌"] },
];
...
const PRESET = ["김준호","이서준","박지우","최민준","정하윤","강서연","조서준","윤예린","임지호","한수아"];
...
export default function NameTagGenerator({ preloadedStudents = [], preloadedLabel = "", onOpenClassPanel, isLoggedIn }: NameTagProps) {
  const { user } = useAuth();
  const loggedIn = isLoggedIn ?? !!user;   // 로그인 여부만으로 전체 기능 게이트
  ...
  // 비로그인 시 글꼴/이모지를 기본값으로 강제 (로그인 풀리는 경우 대비)
  useEffect(() => {
    if (!loggedIn) { setFontId("noto"); setEmojiMode("none"); }
  }, [loggedIn]);
  ...
  const openLogin = () => document.dispatchEvent(new CustomEvent("ssamtool:openLogin"));
        ...
        <div className="bg-white rounded-xl border border-[#E8E0D0] p-5 shadow-sm space-y-4">
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <p className="text-xs font-semibold text-[#4A4A4A]">글꼴</p>
              {!loggedIn && <span className="text-[10px] bg-[#E8E0D0] text-[#6A6A6A] px-2 py-0.5 rounded-full">로그인하면 선택 가능</span>}
            </div>
            <div className="flex flex-wrap gap-2">
              {FONTS.map(f => (
                <button key={f.id} onClick={() => loggedIn && setFontId(f.id)}
                  disabled={!loggedIn}
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              {!loggedIn && <span className="text-[10px] bg-[#E8E0D0] text-[#6A6A6A] px-2 py-0.5 rounded-full">로그인하면 사용 가능</span>}
            </div>
            {loggedIn ? (
              <div className="space-y-3">
                <div className="flex flex-wrap gap-2">
                  {([{ v:"none",label:"없음" },{ v:"smile",label:"😊 고정" },{ v:"random",label:"🎲 랜덤" }] as {v:EmojiMode;label:string}[]).map(({v,label}) => (
              ...
              <button onClick={openLogin}
                className="text-xs text-[#1B4332] underline underline-offset-2 font-semibold">
                로그인하고 이모지 꾸미기 →
              </button>
            )}
          </div>
            ...
            <div className="space-y-4">
              {entries.map(entry => (
                <ScreenCard key={entry.id} entry={entry} fontCss={currentFont.css}
                  ...
                  onPrint={loggedIn ? () => printEntries([entry], currentFont.css, orientation) : undefined}
                />
              ))}
            </div>
          </div>
  ...
  return (
    <div className="border border-[#E8E0D0] rounded-xl overflow-hidden bg-white">
      <div className="flex items-center justify-between px-4 py-2 border-b border-[#E8E0D0] bg-[#FAFAFA]">
        ...
        <div className="flex items-center gap-2">
          {onReshuffle && (
            <button onClick={onReshuffle} className="text-xs text-[#2D6A4F] underline underline-offset-2">이모지 바꾸기</button>
          )}
          {onPrint && (
      ...
      {orientation === "portrait" ? (
        <div className="flex flex-col items-center py-3 gap-0" style={{ fontFamily: fontCss }}>
          ...
          <div className="flex items-center gap-1 w-52"><div className="flex-1 border-t border-dashed border-gray-300"/><span className="text-[8px] text-gray-300">위쪽 접기</span></div>
          ...
          <div className="flex items-center gap-1 w-52"><div className="flex-1 border-t border-dashed border-gray-300"/><span className="text-[8px] text-gray-300">아래 접기</span></div>
        ...
        <div className="flex flex-row items-center py-3 px-3 gap-0 overflow-x-auto" style={{ fontFamily: fontCss }}>
          ...
          <div className="flex flex-col h-20 items-center justify-center flex-shrink-0 w-5">
            ...
            <span className="text-[7px] text-gray-300" style={{ writingMode: "vertical-rl" }}>위쪽</span>
          </div>
          ...
          <div className="flex flex-col h-20 items-center justify-center flex-shrink-0 w-5">
            <div className="h-full border-l border-dashed border-gray-300"/>
            <span className="text-[7px] text-gray-300" style={{ writingMode: "vertical-rl" }}>아래</span>
          </div>
  ...
  return `
    <div style="width:190mm;page-break-after:always;break-after:page;font-family:${fontCss};">
      ...
      <div style="width:190mm;height:0;border-top:1.5px dashed #AAAAAA;position:relative;">
        <span style="position:absolute;right:0;top:3px;font-size:7px;color:#BBBBBB;">위쪽 접기</span>
      </div>
      ...
      <div style="width:190mm;height:0;border-top:1.5px dashed #AAAAAA;position:relative;">
        <span style="position:absolute;right:0;top:3px;font-size:7px;color:#BBBBBB;">아래 접기</span>
      </div>
      ...
      ">
        ${page.map(e => buildLandscapeHTML(e, fontCss)).join(`
          <div style="width:0;height:190mm;border-left:1px dashed #E0E0E0;position:relative;">
            <span style="position:absolute;top:2px;left:3px;font-size:7px;color:#CCCCCC;">점선 자르기</span>
          </div>`)}
      </div>`).join("");
  }
